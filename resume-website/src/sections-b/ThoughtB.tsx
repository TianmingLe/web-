...
export default function ThoughtB() {
  return (
    <section className="px-6 md:px-12 lg:px-20 py-20 max-w-6xl mx-auto">
          ...
          <div className="space-y-10">
            {thoughtData.career.map((item, index) => (
              <div key={index} className={`relative pl-10 b-fade-up b-stagger-${index + 1}`}>
                ...
                <div className="b-card b-card-terracotta p-6">
                  <div className="flex items-center gap-2 mb-3">
                    <span className="b-tag b-tag-terracotta font-b-mono text-xs">
                      Phase {index + 1}
                    </span>
                  </div>
                  <h4 className="font-b-serif text-xl text-b-ink mb-3">{item.phase}</h4>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5">
          {thoughtData.coreAdvantages.map((adv, index) => {
            ...
            return (
              <div
                key={index}
                className={`b-card b-card-sage p-6 text-center b-fade-up b-stagger-${index + 1}`}
              >
      ...
      <div className="mb-16">
        <div className="flex items-center gap-3 mb-4 b-fade-up">
          <Code2 size={22} className="text-b-terracotta" />
          <h3 className="font-b-serif text-2xl md:text-3xl text-b-ink">
            {thoughtData.vibeCoding.title}
          </h3>
        </div>
        <p className="font-b-sans text-b-ink-light text-base leading-relaxed max-w-3xl mb-10 b-fade-up b-stagger-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-5 mb-10">
          {thoughtData.vibeCoding.steps.map((step, index) => (
            <div
              key={index}
              className={`b-card b-card-terracotta p-6 relative overflow-hidden b-fade-up b-stagger-${index + 1}`}
            >
      ...
      <div className="mb-16">
        <div className="flex items-center gap-3 mb-2 b-fade-up">
          <Compass size={22} className="text-b-slate" />
          <h3 className="font-b-serif text-2xl md:text-3xl text-b-ink">
            {thoughtData.techSelection.title}
          </h3>
        </div>
        <p className="font-b-serif text-lg text-b-terracotta italic mb-8 b-fade-up b-stagger-1">
          {thoughtData.techSelection.subtitle}
        </p>

        <div className="b-card b-card-slate p-6 mb-8 b-fade-up b-stagger-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {thoughtData.techSelection.principles.map((p, index) => (
            <div
              key={index}
              className={`b-card b-card-slate p-6 b-fade-up b-stagger-${index + 3}`}
            >
      ...
      <div>
        <div className="flex items-center gap-3 mb-4 b-fade-up">
          <Kanban size={22} className="text-b-terracotta" />
          <h3 className="font-b-serif text-2xl md:text-3xl text-b-ink">
        ...
        <div className="space-y-5 mb-12">
          {thoughtData.projectManagement.competitions.map((comp, index) => (
            <div
              key={index}
              className={`b-card b-card-terracotta p-6 b-fade-up b-stagger-${index + 2}`}
            >
